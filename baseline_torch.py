...
def train_cv(cv=0, seed=42, result_dir="./results"):
    df = prep()
    num_labels = df.label.nunique()
...
class LitOpenSequence(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        f1 = f1_score(y_true=labels.cpu(), y_pred=preds.cpu(), average='macro')
        acc = accuracy_score(y_true=labels.cpu(), y_pred=preds.cpu())
...
def classifier_train(do_lr_find=False):
    ...
    lr = 0.00001
    if do_lr_find:
        model = LitOpenSequence(num_labels=2, lr=lr)
    ...
    model = LitOpenSequence(lr=lr, num_labels=2)
    trainer = pl.Trainer(gpus=1, callbacks=[mc], gradient_clip_val=0.5, logger=wandb_logger)
